...
class AddonTool
{
    ...
    {
        $this->uploadDir = STORAGE_DIR . '/uploads';
    }
    ...
    private function checkRateLimit(): void
    {
        ...
        $ipHash = md5($ip);
        $rateDir = STORAGE_DIR . '/ratelimit';
        if (!is_dir($rateDir)) {
            mkdir($rateDir, 0755, true);
        }
        self::safeChmod($rateDir, 0755);
    ...
    // -------------------------------------------------------------------------
    // Upload validation
    private static function getUploadMaxBytes(): int
    {
        $val  = trim(ini_get('upload_max_filesize') ?: '25M');
    ...
    private static function getUploadMaxDisplay(): string
    {
        $val = ini_get('upload_max_filesize') ?: '25M';
        if (preg_match('/^(\d+)([KMG])?$/i', $val, $m)) {
            $unit = strtoupper($m[2] ?? '');
            return $m[1] . ' ' . ($unit ? $unit . 'B' : 'bytes');
        }
        return $val;
    }
    ...
    private function validateUpload(): string
    {
        ...
        $maxBytes   = self::getUploadMaxBytes();
        $maxDisplay = self::getUploadMaxDisplay();
        ...
        if (!$file || $file['error'] !== UPLOAD_ERR_OK) {
            ...
            $clientMessage = match ($err) {
                UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE => "File too large. Maximum size is {$maxDisplay}.",
                UPLOAD_ERR_NO_FILE => 'No file was received in submission.',
        ...
        if ($file['size'] > $maxBytes) {
            $this->jsonError("File too large. Maximum size is {$maxDisplay}.");
        }
    ...
    private function storeZip(string $tmpPath, string $id): string
    {
        ...
        self::safeChmod($this->uploadDir, 0755);
    ...
    public function generateApprovalToken(array $zip, string $storagePath, string $name, string $email, string $message): string
    {
        ...
        self::safeChmod($tokenFile, 0644);

        return $token;
    }

    public static function peekToken(string $storageDir, string $token): ?array
    {
    ...
    {
        $data = self::peekToken($storageDir, $token);
        if ($data) {
            self::deleteToken($storageDir, $token);
        }
        return $data;
    }
    ...
    public static function handleRequest(): void
    {
        try {
            ...
            if (isset($_GET['token']) && $_SERVER['REQUEST_METHOD'] === 'GET') {
                $token = $_GET['token'];
                $data  = self::peekToken(STORAGE_DIR, $token);
...
<div class="max-w-2xl mx-auto px-4 py-12">
    <h1 class="text-3xl font-bold text-orange-600 mb-6">Addon Approval</h1>';
            ...
            self::logError("Approval error: " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine() . "\n" . $e->getTraceAsString());
            header('Content-Type: text/html; charset=UTF-8', true, 500);
            echo '<!DOCTYPE html><html><body style="font-family:sans-serif;padding:2rem;"><h1>Approval Error</h1><p>' . htmlspecialchars($e->getMessage()) . '</p></body></html>';
            exit;
        }
